...
## Next need to see if the same trees are counted twice
## There are a lot of trees that are counted twice, once per eventID
test <- dplyr::select(dat_canopy, individualID, plotID, taxonID, eventID)
janitor::get_dupes(test)

# Let's look at individual eventID x plotID combinations
test <- dplyr::select(dat_canopy, plotID, eventID) |>
  dplyr::distinct()

## There are different final years for different plots, so I think the
## simplest thing will be to split out the different plots and filter
## for the final eventID for each one separately
## Note that we need to make sure we carry over all the individuals
## Same number of unique individuals in the final eventID compared to all eventIDs
## for each plot
